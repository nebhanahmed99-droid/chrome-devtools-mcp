...
test('validateEnumHomogeneity should return the primitive type of a homogeneous enum', () => {
...
test('validateEnumHomogeneity should throw for heterogeneous enum types', () => {
  assert.throws(() => {
    validateEnumHomogeneity(['a', 1, 'c']);
  }, /Heterogeneous enum types found/);
});

test('validateEnumHomogeneity should throw if enum is empty', () => {
  assert.throws(() => {
    validateEnumHomogeneity([]);
  }, /Enum has no values/);
});

test('generateToolMetrics should map tools correctly and apply transformations', () => {
...
test('generateToolMetrics should handle enums correctly', () => {
  const mockTool: ToolDefinition = {
    name: 'enum_tool',
    description: 'test description',
    annotations: {
